...
async fn main() {
    ...
    let pool = PgPool::connect(database_url)
        .await
        .expect(&format!("fail connect databsae, url is [{}]", database_url));
...
fn create_app<Todo: TodoRepository, Label: LabelRepository>(
    ...
    Router::new()
        ...
        .layer(Extension(Arc::new(todo_repository))) // axumアプリケーションでrepositoryを共有して各ハンドラで受け取れるようにする
...
mod test {
    ...
    use tower::ServiceExt; // oneshotメソッドに必要
